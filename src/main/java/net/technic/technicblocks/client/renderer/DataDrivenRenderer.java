...
public abstract class DataDrivenRenderer implements ISimpleBlockRenderingHandler {
    ...
    protected void renderFace(ForgeDirection face, float startX, float startY, float endX, float endY, float depth, BlockTextureScheme textureScheme, IRenderContext posContext, RenderBlocks renderer, ForgeDirection upDir) {
        ...
        uvList.add(new Vector2f(icon.getInterpolatedU(startX), icon.getInterpolatedV(startY)));
        uvList.add(new Vector2f(icon.getInterpolatedU(startX), icon.getInterpolatedV(endY)));
        uvList.add(new Vector2f(icon.getInterpolatedU(endX), icon.getInterpolatedV(endY)));
        uvList.add(new Vector2f(icon.getInterpolatedU(endX), icon.getInterpolatedV(startY)));
