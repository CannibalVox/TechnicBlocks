...
public class RedstoneActivationBehavior extends BlockBehavior implements IBlockPlacementBehavior, INeighborUpdateBehavior, IBlockTickBehavior {

    private String targetBlock;

    public RedstoneActivationBehavior(String[] args) {
        super(args);

        if (args.length < 1) {
            throw TechnicBlocks.getProxy().createParseException("'redstoneOn' behaviors should have at least one parameter: the name of the block to transform to.");
        }
